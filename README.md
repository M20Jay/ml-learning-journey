# ML Learning Journey — Martin James
### Data Analyst → ML Engineer | Nairobi, Kenya 🇰🇪

Structured ML learning notebooks following a
curriculum from EDA to Generative AI.
Built in public — one notebook at a time.

---

### 📚 ML Learning Journey — Building In Public

| # | Notebook | Dataset | Status |
|---|----------|---------|--------|
| 01 | EDA and Data Exploration | Titanic | ✅ Complete |
| 02 | Data Preprocessing and Cleaning | Titanic | ✅ Completen |
| 03 | Classification Models | Titanic | ⏳ Coming Soon |
| 04 | Regression Models | House Prices | ⏳ Coming Soon |
| 05 | Model Evaluation and ROC Curves | Titanic | ✅ Complete |
| 06 | Clustering and Unsupervised Learning | Mall Customers | ⏳ Coming Soon |
| 07 | Neural Networks | MNIST | ⏳ Coming Soon |
| 08 | TensorFlow and Keras | MNIST | ⏳ Coming Soon |
| 09 | NLP Fundamentals | Twitter Sentiment | ⏳ Coming Soon |
| 10 | Generative AI and RAG | Custom Documents | ⏳ Coming Soon |

---

## 🚀 Portfolio Projects

| Project | Description | Status |
|---------|-------------|--------|
| Project 1 — Customer Churn Prediction | Classification · SMOTE · FastAPI | 🔄 In Progress |
| Project 2 — Full Stack Dashboard | SQL · Python · Power BI | ⏳ Coming Soon |
| Project 3 — RAG Chatbot | GenAI · LangChain · HuggingFace | ⏳ Coming Soon |

---

## 🗓️ Timeline
**Started:** March 2026
**Target:** September 2026

---

## 🛠️ Tools
Python · scikit-learn · TensorFlow · Pandas ·
SQL · Power BI · Tableau · Google Colab · GitHub

---

## 📫 Connect
**GitHub:** [@M20Jay](https://github.com/M20Jay)
**LinkedIn:** [martin-james-nganga](https://www.linkedin.com/in/martin-james-nganga)
**Email:** ngangam93@gmail.com

---
*Building in public — every commit is progress 🌙🚀*
```
